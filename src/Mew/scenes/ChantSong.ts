...
import { ChantSongViewerData } from "../Resource/ViewerResource/ChantSong";
import * as ChantSongProductData from "../Resource/ProductResource/ChantSong";
import { Home } from "./Home";
import PIXISound from "pixi-sound";

export class ChantSong extends SceneBase {
  ...
  async onInit() {
    ...
    await this.loadViewerResource(ChantSongViewerData);
    await this.loadProductResource(ChantSongProductData[this.productName]);
  }
  async onStart() {
  ...
  dimmed() {
    ...
    const title = new PIXI.Sprite(
      ResourceManager.Handle.getViewerResource(`chantsong_title.png`).texture
    );
    title.position.y = 10;
