...
import { SingAlongViewerData } from "../Resource/ViewerResource/SingAlong";
import * as SingAlongProductData from "../Resource/ProductResource/SingAlong";
import { Home } from "./Home";
import PIXISound from "pixi-sound";

export class SingAlong extends SceneBase {
  ...
  async onInit() {
    ...
    await this.loadViewerResource(SingAlongViewerData);
    await this.loadProductResource(SingAlongProductData[this.productName]);
  }
  async onStart() {
    this.mControllerFlag = false;

    /**비디오 생성 */
    this.mVideo = new Video(this.productName, `singAlong.mp4`);
  ...
  dimmed() {
    ...
    const title = new PIXI.Sprite(
      ResourceManager.Handle.getViewerResource(`singalong_title.png`).texture
    );
    title.position.y = 10;
